...
final class StoreEngineDevelopment implements StoresDevelopment
{
    ...
    public function handle(
        ...
        $this->models = $this->getEngines($entities);
     ...
     * @return Collection<string, EngineSeason>
     */
    private function getEngines(Collection $entities): Collection
    {
        $ids = $entities->map(fn(DevelopmentEntity $entity) => $entity->id);

        return EngineSeason::query()
            ->whereIn('id', $ids)
            ->get()
            ->mapWithKeys(fn(EngineSeason $engine) => [$engine->id => $engine]);
    }
}
